        ...
        if byte in pixeldata[byte_index + 1 : byte_index + distance]:
            ...
            for i in range(distance):
                ...
                if pixeldata[byte_index + index] == byte:
                    ...
                    next_bytes = pixeldata[byte_index + 1 : byte_index + round(index/2) + 2];
                    next_bytes_mean = sum(next_bytes) / len(next_bytes);

                    new_byte = round(byte+last_byte+next_bytes_mean) % 256;
